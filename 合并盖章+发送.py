...
# 全局变量：学号范围与目标路径映射（从send_single_file提取）
STUDENT_ID_RANGES = {
...
# 全局变量：印章路径（从process_single_file提取）
...
def check_target_paths():
    """检查所有目标发送路径和印章路径是否可用"""
    # 检查发送目标路径
    for path in STUDENT_ID_RANGES.values():
        try:
            if not os.path.exists(path):
                print(f"错误：目标路径不存在 - {path}")
                return False
            # 验证访问权限（尝试列出目录）
            os.listdir(path)
        except Exception as e:
            print(f"错误：无法访问目标路径 {path} - {str(e)}")
            return False

    # 检查印章路径
    for stamp_path in [SUCCESS_STAMP, FAIL_STAMP]:
        if not os.path.exists(stamp_path):
            print(f"错误：印章文件不存在 - {stamp_path}")
            return False
        try:
            ...
            print(f"错误：无法访问印章文件 {stamp_path} - {str(e)}")
            return False
